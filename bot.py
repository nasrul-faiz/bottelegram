...
class Bot(Client):
    ...
    async def start(self, **kwargs):
        ...
        # Initialize db_channel (required for bot functionality)
        if CHANNEL_ID and CHANNEL_ID != 0:  # Only check if CHANNEL_ID is set (non-zero)
            try:
            ...
            # Create a dummy db_channel object to prevent AttributeError in plugins
            self.db_channel = None
            self.LOGGER(__name__).warning(
                "⚠️  CHANNEL_ID tidak dikonfigurasi! Bot berjalan dalam mode demo saja."
            )
